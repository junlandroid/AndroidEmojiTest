...
/**
 * EmoticonsBoard 最终 extend RelativeLayout
 * 表情键盘，包含以下部分：
 * 1.输入框上面的分割线
 * 2.输入框一整行区域
 * 3.输入框下面的分割线
...
public class EmoticonsBoard extends AutoHeightLayout implements View.OnClickListener,
    ...
    protected EmoticonsFuncView emoticonsFuncView; //ViewPager
    protected EmoticonsIndicator emoticonsIndicator; //表情ViewPager 指示器
    protected EmoticonsToolBar emoticonsToolBar; // 不同的表情切换tab，
    ...
    public EmoticonsBoard(Context context, AttributeSet attrs) {
        ...
        inflateKeyboardBar();// 填充所有的键盘/表情区域
        initView(); // 初始化控件
        initFuncView();//
    }
    ...
    protected void initFuncView() {
        initEmoticonFuncView(); //填充并初始化除了输入框那一行区域之外的所有键盘区域
        initEditView(); // 初始输入框
    }
    ...
    protected void initEmoticonFuncView() {
        ...
        emoticonsFuncView = ((EmoticonsFuncView) findViewById(R.id.view_epv)); //ViewPager
